...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  $sql = "SELECT * FROM newemployee_register WHERE register_id=$register_id";
  ...
  $employee_number = $_row["employee_number"];
  $first_name = $_row["first_name"];
  $middle_name = $_row["middle_name"];
  $surname = $_row["surname"];
  $suffix = $_row["suffix"];
  $date_birth = $_row["date_birth"];
  $gender = $_row["gender"];
  $nationality = $_row["nationality"];
  $civil_status = $_row["civil_status"];
  $department = $_row["department"];
  $designation = $_row["designation"];
  $qual_dept_status = $_row["qual_dept_status"];
  $employee_status = $_row["employee_status"];
  $paydate = $_row["paydate"];
  $contact_number = $_row["contact_number"];
  $email = $_row["email"];
  $other = $_row["other"];
  $social_acc = $_row["social_acc"];
  $address1 = $_row["address1"];
  $address2 = $_row["address2"];
  $city_muni = $_row["city_muni"];
  $state_pro = $_row["state_pro"];
  $country = $_row["country"];
  $zip_code = $_row["zip_code"];

} else {
  //POST Method: update the table data
  ...
  do {
    if (empty($register_id) || empty($employee_number) || empty($first_name) || empty($middle_name) || empty($surname) || empty($date_birth) || empty($gender) || empty($nationality) || empty($civil_status) || empty($department) || empty($designation) || empty($qual_dept_status) || empty($employee_status) || empty($paydate) || empty($contact_number) || empty($email) || empty($other) || empty($social_acc) || empty($address1) || empty($address2) || empty($city_muni) || empty($state_pro) || empty($country) || empty($zip_code)) {
      $errorMessage = "All the fields are required";
      break;
    }

    $sql = "UPDATE newemployee_register " .
    "SET employee_number='$employee_number',first_name='$first_name',middle_name='$middle_name',surname='$surname',suffix='$suffix',date_birth='$date_birth',gender='$gender',nationality='$nationality',civil_status='$civil_status',department='$department',designation='$designation',qual_dept_status='$qual_dept_status',employee_status='$employee_status',paydate='$paydate',contact_number='$contact_number',email='$email',other='$other',social_acc='$social_acc',address1='$address1',address2='$address2',city_muni='$city_muni',state_pro='$state_pro',country='$country',zip_code='$zip_code'" .
    ...
    $successMessage = "Successfully Updated Employee Info!";
  }while (false);
...
<head>
  ...
  <title>Edit And Update Employee Info</title>
...
<body>
  <div class="container">
    <header>Edit And Update Employee Info</header>
